...
def _parse_artwork(movie, collection, urlbases, language):
    ...
    original_language = movie.get('original_language') if movie else None
    posters = []
    landscape = []
    logos = []
    fanart = []
    ...
    if 'images' in movie:
        posters = _build_image_list_with_fallback(movie['images']['posters'], urlbases, language, original_language=original_language)
        landscape = _build_image_list_with_fallback(movie['images']['backdrops'], urlbases, language, original_language=original_language)
        logos = _build_image_list_with_fallback(movie['images']['logos'], urlbases, language, original_language=original_language)
...
def _build_image_list_with_fallback(imagelist, urlbases, language, language_fallback='en', original_language=None):
    # Preference order: user's language, English fallback, original movie language, anything.
    images = []
    seen = set()
    def add_for(lang):
        for img in _build_image_list(imagelist, urlbases, [lang]):
            if img['url'] not in seen:
                seen.add(img['url'])
                images.append(img)
    if language:
        add_for(language)
    if language_fallback != language:
        add_for(language_fallback)
    if original_language and original_language != language and original_language != language_fallback:
        add_for(original_language)
    if not images:
